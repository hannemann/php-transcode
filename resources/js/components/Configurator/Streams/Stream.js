...
class Stream extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.streams.forEach((stream) => {
            ...
            node.querySelector("transcode-configurator-stream-buttons").item =
                stream;
            this.itemContainer.append(this.initItem(stream, node));
            node.querySelectorAll("[data-toggle]").forEach((t) =>
                t.addEventListener("click", this.toggleView),
            );
        });
    ...
    handleToggleStream(e) {
        e.detail.origin.closest(".stream").dataset.active =
            e.detail.item.active.toString();
    }
}
