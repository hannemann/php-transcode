...
class Streams extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.video?.find((v) => v.active) ||
            this.video?.forEach((v, k) => (v.active = k === 0));
        this.audio?.find((a) => a.active) ||
            this.audio?.forEach(
                (v, k) =>
                    (v.active = PREFERRED_LANGUAGES.includes(v.tags?.language)),
            );
        this.audio?.find((a) => a.active) ||
    ...
    getVideoStreams() {
        const node = document.createElement(
            "transcode-configurator-stream-video",
        );
    ...
    getAudioStreams() {
        const node = document.createElement(
            "transcode-configurator-stream-audio",
        );
    ...
    getSubtitleStreams() {
        const node = document.createElement(
            "transcode-configurator-stream-sub",
        );
    ...
    getDataStreams() {
        const node = document.createElement(
            "transcode-configurator-stream-data",
        );
    ...
    get video() {
        return this.items
            ?.filter((i) => i.codec_type === TYPE_VIDEO)
            .sort((a, b) => a.index > b.index);
    }

    get audio() {
        return this.items
            ?.filter((i) => i.codec_type === TYPE_AUDIO)
            .sort((a, b) => a.index > b.index);
    }

    get subs() {
        return this.items
            ?.filter((i) => i.codec_type === TYPE_SUB)
            .sort((a, b) => a.index > b.index);
    }

    get data() {
        return this.items
            ?.filter((i) => i.codec_type === TYPE_DATA)
            .sort((a, b) => a.index > b.index);
    }
}
