...
class Video extends Stream {
    initItem(stream, node) {
        node.querySelector(".short-description").append(
            document.createTextNode(this.getShortDescription(stream)),
        );
        node.querySelector('.long-description[data-desc="index"]').append(
            document.createTextNode(this.getIndexDescription(stream)),
        );
        node.querySelector('.long-description[data-desc="codec"]').append(
            document.createTextNode(this.getCodecDescription(stream)),
        );
        node.querySelector('.long-description[data-desc="dim"]').append(
            document.createTextNode(this.getDimensionDescription(stream)),
        );
        return node;
    }
    ...
    getShortDescription(stream) {
        ...
        const targetCodec =
            Object.values(VIDEO_CODECS).find(
                (c) => c.v === stream.transcodeConfig.codec,
            )?.l || "";
        const qp = this.getQp(stream);
        const aspect = `DAR: ${stream.transcodeConfig.aspectRatio}`;
        return (
            `Stream #0:${stream.index} ${stream.codec_name} ${dim}` +
    ...
    getCodecDescription(item) {
        ...
        const codec =
            [item.codec_tag_string, item.codec_tag]
                .filter((i) => i)
                .join(" / ")
                ?.wrap("(", ")") || "";
        const pixFmt = item.pix_fmt || "";
        const color = [item.color_range, item.color_space, item.field_order]
            .filter((i) => i)
            .join(", ")
            ?.wrap("(", ")");
    ...
    getDimensionDescription(item) {
        ...
        const aspect = [
            item.sample_aspect_ratio?.wrap("SAR "),
            item.display_aspect_ratio?.wrap("DAR "),
        ]
            .filter((i) => i)
            .join(" ")
            ?.wrap("[", "]");
        let frameRate = item.avg_frame_rate || "N/A";
        if (item.avg_frame_rate?.includes("/")) {
            const parts = item.avg_frame_rate
                .split("/")
                .map((f) => parseFloat(f));
            frameRate = parts[0] / parts[1];
        }
        frameRate = `${frameRate} fps`;
        let tbr = item.r_frame_rate || "N/A";
        if (item.r_frame_rate?.includes("/")) {
            const parts = item.r_frame_rate
                .split("/")
                .map((f) => parseFloat(f));
            tbr = parts[0] / parts[1];
        }
    ...
    getDefaultQp(item) {
        const qp =
            Object.values(VIDEO_CODECS).find(
                (c) => c.v === item.transcodeConfig.codec,
            )?.qp || "";
